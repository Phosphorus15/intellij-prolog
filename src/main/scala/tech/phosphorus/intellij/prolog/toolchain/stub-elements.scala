...
class PrologPredicateType(debugName: String) extends PrologStubElementType[PrologPredicateStub, PrologPredicateId](debugName) {
  ...
  override def createStub(psiT: PrologPredicateId, parentStub: StubElement[_ <: PsiElement]): PrologPredicateStub = new PrologPredicateStubImpl(parentStub, psiT.getText)
